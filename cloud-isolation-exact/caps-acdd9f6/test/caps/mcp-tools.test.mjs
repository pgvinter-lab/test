...
test("MCP caps tools acceptance", async (t) => {
  ...
  try {
    ...
    await t.test("retains every pre-existing Bridge tool (existing-tool preservation)", () => {
    ...
    await t.test("non-Code refresh is inert and returns an exact A2A request (non-Code byte parity)", async () => {
      const before = capabilitySnapshot(databasePath);
      const response = await cowork.client.callTool({
        name: "caps_refresh",
    ...
    const codeClientConn = await connect(
      "code",
      "claude_desktop_code",
      "claude-code",
    );

    await t.test("a held 08A lock returns bounded non-mutating result with provenance", async () => {
      const lockPath = path.join(capsDirectory, "refresh.lock");
      ...
      fs.writeFileSync(lockPath, JSON.stringify(lockProvenance));
      const beforeSnapshot = capabilitySnapshot(databasePath);

      const response = await codeClientConn.client.callTool({
        name: "caps_refresh",
        arguments: { lane: "config-crawl" },
      });
      ...
      assert.match(result.message, /already_running/);

      const afterSnapshot = capabilitySnapshot(databasePath);
      assert.strictEqual(afterSnapshot, beforeSnapshot);
      const afterLock = fs.readFileSync(lockPath, "utf8");
      assert.strictEqual(afterLock, JSON.stringify(lockProvenance));

      fs.rmSync(lockPath, { force: true });
    });

    await t.test("separately prove the fast in-process guard", async () => {
      const p1 = codeClientConn.client.callTool({
        name: "caps_refresh",
        arguments: { lane: "config-crawl" },
      });
      const p2 = codeClientConn.client.callTool({
        name: "caps_refresh",
        arguments: { lane: "config-crawl" },
      });
      const results = await Promise.all([p1, p2]);
      const errors = results.filter((r) => r.isError);
      assert.ok(errors.length >= 1, "at least one concurrent call failed");
      const texts = errors.map((e) => JSON.parse(e.content[0].text));
      assert.ok(
        texts.some((t) => t.message.includes("caps_refresh_in_progress")),
        "fast guard prevented concurrent execution",
      );
    });

    await t.test("Code refresh returns a unified report schema with lifecycle parity", async () => {
      const response = await codeClientConn.client.callTool({
        name: "caps_refresh",
        arguments: { lane: "config-crawl" },
      });
      assert.ok(!response.isError, "refresh failed unexpectedly");
      const result = JSON.parse(response.content[0].text);
      ...
      assert.strictEqual(result.reports.schema, "bridge-caps-refresh-report-v1");
      assert.strictEqual(result.reports.lifecycle_status, "terminal-success");
      assert.ok(result.reports.lane_outcomes.config.observation_count >= 1);
