...
import * as capsCli from "./caps/cli.js";
...
Caps capability registry
  caps status                           Show DB path, migration state, row counts, and refresh state
  caps search <query>                   Search the local catalog (paid results last)
  caps get <id>                         Get full capability record
  caps refresh [--lane config|probe|mcpservers|all] Update the local database from the central orchestrator
  caps backfill [--resume]              Paced backfill of historical definitions
...
async function main(): Promise<void> {
  ...
  switch (cmd) {
    ...
    case "caps": {
      await capsCli.runCapsCommand(positional, flags);
      break;
    }
