...
import { runCapsRefresh, type CapsRefreshLane, type CapsRefreshStage } from "./refresh.js";
import type {
  CensusAttribution,
...
export class CapsMcpRuntime {
  ...
  public async refresh(
    ...
    try {
      const internalLane: CapsRefreshLane = lane === "config-crawl"
        ? "config"
        : lane === "stdio-probe"
          ? "probe"
          : lane === "mcpservers"
      ...
      return {
        ...
        lane,
        mutated: true,
        started_at: report.start_at,
        completed_at: report.end_at ?? new Date().toISOString(),
        reports: report,
      };
    } finally {
