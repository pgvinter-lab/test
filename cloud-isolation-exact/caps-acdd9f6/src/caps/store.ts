...
export class CapsStore {
  ...
  public recordJudgmentVerdict(
    verdicts: Array<{
      ...
      judgment_surface: string;
    }>
  ): { appliedCount: number; idempotentCount: number; notInDbCount: number } {
    return this.transaction(() => {
      let appliedCount = 0;
      ...
      for (const v of verdicts) {
        const row = this.db.prepare(`SELECT judgment_verdict, judgment_model, judgment_at, judgment_reason, judgment_surface FROM available_for_install WHERE id = ?`).get(v.id) as any;
        if (!row) {
          notInDbCount++;
          continue;
        }

        if (row.judgment_verdict === v.judgment_verdict &&
            row.judgment_model === v.judgment_model &&
            row.judgment_at === v.judgment_at &&
            row.judgment_reason === v.judgment_reason &&
            row.judgment_surface === v.judgment_surface) {
          idempotentCount++;
          continue;
        }
      ...
      return { appliedCount, idempotentCount, notInDbCount };
    });
  }
}
