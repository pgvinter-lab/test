...
export type JudgmentVerdict = 'immediate-need' | 'watch' | 'no';

export interface JudgmentRequestDiffCandidate {
  id: string;
  name: string;
  kind: CapabilityKind;
  pricing: CapabilityPricing;
...
export interface JudgmentRequest {
  ...
  diff: {
    new_available: JudgmentRequestDiffCandidate[];
    status_flips: string[];
    watch_deltas: string[];
  };
  rules: {
    free_first: boolean;
    paid_ask_first: boolean;
  };
  response_contract: {
    model: string;
    judged_at: string;
    verdicts: Array<{
      ...
      reason: string;
      pricing_ack: CapabilityPricing;
    }>;
  };
}

export interface JudgmentResponseVerdict {
  id: string;
  verdict: JudgmentVerdict;
  reason: string;
  pricing_ack: CapabilityPricing;
...
export interface JudgmentResponse {
  model: string;
  judged_at: string; // ISO-8601 UTC
  verdicts: JudgmentResponseVerdict[];
}
