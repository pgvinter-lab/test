...
test("A2A model and effort selection reach the exact subscription CLI invocation", async () => {
  const fx = fixture();
  try {
    const result = await fx.facade.a2aSend({
      project: fx.project,
      target: "codex",
      prompt: "Return selected model sentinel.",
      idempotencyKey: "antigravity-a2a-selected-model-1",
      model: "gpt-5.6-sol",
      effort: "max",
    });
    assert.equal(result.state, "completed");
    ...
    assert.deepEqual(
      fx.calls[0].args,
      ["exec", "--skip-git-repo-check", "--model", "gpt-5.6-sol", "--config", 'model_reasoning_effort="max"'],
    );
