...
test("model and effort selection are translated into each subscription CLI's native flags", () => {
  assert.deepEqual(
    buildPeerArgv(peerCliSpec("antigravity"), "work", {
  ...
  assert.deepEqual(
    buildPeerArgv(peerCliSpec("claude"), "work", {
      model: "claude-opus-5",
      effort: "max",
    }),
    ["-p", "--model", "claude-opus-5", "--effort", "max"],
  );
  assert.deepEqual(
    buildPeerArgv(peerCliSpec("codex"), "work", {
      model: "gpt-5.6-sol",
      effort: "ultra",
    }),
    ["exec", "--skip-git-repo-check", "--model", "gpt-5.6-sol", "--config", 'model_reasoning_effort="ultra"'],
  );
});

test("model and effort selection rejects cross-provider identifiers and unsupported effort", () => {
  assert.throws(
    () => buildPeerArgv(peerCliSpec("claude"), "work", { model: "gpt-5.6-sol" }),
    /a2a_peer_model_invalid/u,
  );
  assert.throws(
    () => buildPeerArgv(peerCliSpec("antigravity"), "work", { effort: "max" }),
    /a2a_peer_effort_invalid/u,
  );
  assert.throws(
    () => buildPeerArgv(peerCliSpec("codex"), "work", { model: "--api-key" }),
    /a2a_peer_model_invalid/u,
  );
});
