...
ALTER TABLE mailbox_deliveries RENAME TO mailbox_deliveries_v1;
ALTER TABLE mailbox_messages RENAME TO mailbox_messages_v1;

CREATE TABLE mailbox_messages (
  message_id TEXT PRIMARY KEY,
  project_id TEXT NOT NULL,
  sender_json TEXT NOT NULL CHECK (json_valid(sender_json)),
  schema_version TEXT NOT NULL CHECK (schema_version IN ('bridge-mailbox-v1','bridge-mailbox-v2')),
  provider TEXT NOT NULL CHECK (provider IN ('chatgpt','antigravity','gemini')),
...
SELECT
  message_id, project_id, sender_json, 'bridge-mailbox-v1', provider, mode, priority, sensitivity,
  status, created_at, expires_at, prompt_sha256, approval_ref,
  envelope_sha256, envelope_relative_path, ready_relative_path, attempt,
  max_attempts, active_delivery_id, response_sha256, response_relative_path,
  last_error, updated_at
FROM mailbox_messages_v1;
...
SELECT
  delivery_id, message_id, 'bridge-mailbox-v1', provider, consumer_id, delivery_token_hash, status,
  claimed_at, lease_expires_at, response_id, response_created_at,
  response_content_sha256, response_sha256, response_relative_path, error_code,
  completed_at, updated_at
FROM mailbox_deliveries_v1;

DROP TABLE mailbox_deliveries_v1;
DROP TABLE mailbox_messages_v1;
...
SET value = 'bridge-mailbox-v2'
WHERE key = 'schema_version' AND value = 'bridge-mailbox-v1';

INSERT OR REPLACE INTO mailbox_metadata(key, value)
VALUES ('legacy_provider', 'gemini');
