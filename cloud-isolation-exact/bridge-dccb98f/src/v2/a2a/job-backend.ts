...
export class JobBackedA2ATaskBackend implements A2ATaskBackend {
  ...
  async send(params: MessageSendParams, caller: A2ACaller): Promise<A2ATask> {
    ...
    if (targetPeer) {
      const selection = requestedExecutionSelection(params, targetPeer);
...
function requestedExecutionSelection(
  params: MessageSendParams,
  targetPeer: CommandCenterPeer,
): { model?: string; effort?: string } {
  const metadata = { ...(params.message.metadata || {}), ...(params.metadata || {}) };
  const model = metadata.bridgeModel;
  const effort = metadata.bridgeEffort;
  if (model !== undefined) {
    invariant(typeof model === "string" && model.length >= 1 && model.length <= 120, "a2a_model_invalid", {
      targetPeer,
    });
  }
  if (effort !== undefined) {
    invariant(
      typeof effort === "string" && ["low", "medium", "high", "xhigh", "max", "ultra"].includes(effort),
      "a2a_effort_invalid",
      { targetPeer, effort },
    );
  }
