...
export interface PeerCliSpec {
  ...
  /** Existing model behavior used when the caller does not select a model. */
  readonly defaultModel?: string;
}

export interface PeerExecutionSelection {
...
export function buildPeerArgv(
  spec: PeerCliSpec,
  prompt: string,
  selection: PeerExecutionSelection = {},
): readonly string[] {
  const selected = validatePeerExecutionSelection(spec.peer, selection);
  const runtimeFlags = selectionFlags(spec, selected);
  const flags = spec.peer === "antigravity"
    ? [spec.flags[0], ...runtimeFlags, ...spec.flags.slice(1)]
    : [...spec.flags, ...runtimeFlags];
  if (spec.promptMode === "stdin") return flags;
  invariant(spec.promptFlag, "a2a_peer_dispatch_prompt_flag_missing", { peer: spec.peer });
  return [...flags, `${spec.promptFlag}=${prompt}`];
}
...
export async function dispatchToPeer(
  ...
  const args = buildPeerArgv(spec, prompt, selection);
...
function validatePeerExecutionSelection(
  peer: CommandCenterPeer,
  selection: PeerExecutionSelection,
): PeerExecutionSelection {
  const model = selection.model?.trim();
  const effort = selection.effort?.trim().toLowerCase();
  if (model) {
    invariant(model.length <= 120, "a2a_peer_model_invalid", { peer });
    const validModel = peer === "antigravity"
      ? /^gemini-[a-z0-9.-]+$/u.test(model)
      : peer === "claude"
        ? /^(?:claude-[a-z0-9.-]+|fable|opus|sonnet|haiku)$/u.test(model)
        : /^gpt-[a-z0-9.-]+$/u.test(model);
    invariant(validModel, "a2a_peer_model_invalid", { peer, model });
  }
  if (effort) {
    const allowed = peer === "antigravity"
      ? ["low", "medium", "high"]
      : peer === "claude"
        ? ["low", "medium", "high", "xhigh", "max"]
        : ["low", "medium", "high", "xhigh", "max", "ultra"];
    invariant(allowed.includes(effort), "a2a_peer_effort_invalid", { peer, effort });
  }
  return {
    ...(model ? { model } : {}),
    ...(effort ? { effort } : {}),
...
function selectionFlags(spec: PeerCliSpec, selection: PeerExecutionSelection): readonly string[] {
  const model = selection.model || spec.defaultModel;
  const effort = selection.effort;
  if (spec.peer === "antigravity") {
    return [
      ...(model ? ["--model", model] : []),
