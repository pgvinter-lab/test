...
export class LegacyBridgeFacade {
    ...
    try {
      ...
      try {
        ...
        task = await client.send({
          ...
          metadata: {
            bridgeTargetPeer: args.target,
            ...(args.model ? { bridgeModel: args.model } : {}),
            ...(args.effort ? { bridgeEffort: args.effort } : {}),
          },
        });
      } finally {
        await server.close();
  ...
  private assertCommandIdempotency(
    ...
    for (const [key, value] of Object.entries(expected)) {
      invariant(key in request && canonicalEqual(request[key], value), "idempotency_key_reused", {
        operation,
        idempotencyKey,
      });
    }
